...
endpoints. Its primary function is to assess the "slop" or the tendency of language
models to generate repetitive or overly common phrases. It achieves this by:
1. Loading a series of prompts from a specified YAML file.
2. Sending these prompts to the target API endpoint.
3. Collecting and concatenating the model's responses.
4. Normalizing the text (lowercase, remove punctuation, extra spaces).
5. Tokenizing the text into words.
6. Filtering out a predefined list of common English stop words.
7. Calculating and displaying the frequency of the top N unigrams, bigrams,
   and trigrams found in the generated text, which helps identify repetitive patterns.
...
- main(): The core function that orchestrates the entire benchmarking process. It handles:
    ...
    - Constructing and sending HTTP requests (supporting batching and streaming)
      to the OpenAI-compatible API.
    - Processing API responses, including handling potential errors and extracting
      the generated text.
    - Normalizing the aggregated text.
    - Tokenizing the text.
...
- prompts (list): Contains the prompts loaded from the YAML file.
- headers (dict): Stores HTTP headers for API requests, primarily for authorization.
- combined_text (str): Accumulates all text generated by the language model.
- batch_payload / payload (dict): Dictionaries representing the JSON body of API requests.
- resp (requests.Response): Object representing the HTTP response from the API.
- data (dict/list): Parsed JSON data from the API response.
- words (list): List of individual words (tokens) after text processing.
- stop_words (set): A predefined set of common English words (articles, prepositions, etc.)
  that are filtered out before frequency analysis.
- unigram_counts, bigram_counts, trigram_counts (collections.Counter): Objects used to
  store the frequency of each unigram, bigram, and trigram.

Parameters (Command-Line Arguments):
- --server_url (str): Base URL of the OpenAI-compatible API endpoint.
  (Default: "http://localhost:8888/v1")
- --api_key (str): Bearer token for API authentication.
  (Default: value of OPENAI_API_KEY environment variable)
- --model (str): Name of the language model to be used for completions.
  (Default: 'text-davinci-003')
- --temperature (float): Controls randomness in generation (0.0-2.0).
  (Default: 0.7)
- --top_k (int): Samples from the k most likely next tokens. 0 disables it.
  (Default: 0)
...
- --presence_penalty (float): Penalizes new tokens based on whether they appear in the text so far.
  (Default: 0.0)
- --prompts_yaml (str): Path to the YAML file containing the list of prompts.
  (Default: "{script_directory}/elarabench_prompts.yaml")
- --max_tokens (int): Maximum number of tokens to generate for each prompt.
  (Default: 300)
- --repeat (int): Number of times to repeat each prompt.
  (Default: 1)
- --repeat_until_word_count (int): Continue generating responses for prompts until the
  total word count (after filtering) reaches this value. 0 means no limit.
  (Default: 0)
- --batch_size (int): Number of prompts to send in a single API request. If set to 1,
  the script uses streaming for individual prompts.
  (Default: 5)
- --output_file (str): Path to the file where the combined raw output text will be saved.
  (Default: "output.txt")
- --number_of_words (int): The number of top unigrams, bigrams, and trigrams to display
  in the frequency analysis. (Default: 25)

Configurations:
- current_dir (str): The absolute path to the directory where the script is located.
  Used to resolve the default path for `prompts_yaml`.
- API Endpoint Path: Hardcoded as "/completions" which is appended to `server_url`.
- Request Headers: Primarily includes 'Authorization': 'Bearer <api_key>'.
- Request Payload: JSON structure containing model parameters and prompts, tailored for
  batched or single/streaming requests.

Hardcoded Variables:
- instruct_template (str): A specific template for formatting prompts, likely intended for
  instruction-tuned models (e.g., Llama-based models). It wraps the user's prompt with
  system and assistant role indicators.
  Example: "<|begin_of_text|><|start_header_id|>system<|end_header_id|>..."
- stop_words (set): A comprehensive set of common English words (articles, pronouns,
  prepositions, conjunctions, common verbs, etc.) that are excluded from the
  n-gram frequency analysis to better highlight unique or repetitive content
  generated by the model.
- Default values for all command-line arguments (see Parameters section).
- Streaming response parsing logic: Identifiers like 'data: ' and 'data: [DONE]'
  are used to process Server-Sent Events (SSE) from streaming API responses.
"""
#!/usr/bin/env python3
"""
